...
the extended vocab (vocab_size=50262, adds <calc>/</calc> and the chat
turn tokens) so you can continue fine-tuning it on
tools/prepare_calc_data.py and/or chat/prepare_chat_*.py's data instead of
training that behavior from scratch.

Copies over all existing rows of the token embedding / lm_head weight
(they're tied, so it's one tensor) and randomly initializes the 5 new rows
...
from tools.tokenizer import VOCAB_SIZE as EXTENDED_VOCAB_SIZE
...
def main():
    ...
    if old_vocab >= EXTENDED_VOCAB_SIZE:
        raise ValueError(
            f"checkpoint vocab_size={old_vocab} is already >= tool vocab_size="
            f"{EXTENDED_VOCAB_SIZE}; nothing to resize"
        )
    ...
    new_wte = torch.empty(EXTENDED_VOCAB_SIZE, n_embd, dtype=old_wte.dtype)
    ...
    cfg.vocab_size = EXTENDED_VOCAB_SIZE
    ...
    print(f"resized vocab {old_vocab} -> {EXTENDED_VOCAB_SIZE}, wrote {args.out_ckpt}")
